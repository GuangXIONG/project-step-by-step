...
class PageViewManager(models.Manager):
	...
	def get_projects(self):
		return self.get_queryset().projects()
...
class PageView(models.Model):
	...
	primary_content_type = models.ForeignKey(
		ContentType, related_name='primary_obj', null=True, blank=True
	)
	...
	secondary_content_type = models.ForeignKey(
		ContentType, related_name='secondary_obj', null=True, blank=True
	)
...
def page_view_receiver(sender, **kwargs):
	page_path = kwargs.get('page_path')
	primary_obj = kwargs.get('primary_obj')
	secondary_obj = kwargs.get('secondary_obj')
	user = sender
...
page_view.connect(page_view_receiver)
